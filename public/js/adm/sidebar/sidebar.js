...
webix.type(webix.ui.tree, {
	...
	arrow: function(obj, common){
		...
		for (var i=1; i<=obj.$level; i++){
			if (i==obj.$level && obj.$count){
				var className = "webix_sidebar_dir_icon webix_icon fa-angle-"+(obj.open?"down":"right");
	...
	icon:function(obj, common){
		if(obj.icon)
			return "<span style='margin:10px auto;' class='webix_icon webix_sidebar_icon fa-"+obj.icon+"'></span>";
		return "";
	}
});